...
{
    /// <summary>
    /// Truncate a table (only if it exists)
    /// </summary>
    public class DestinationTableTruncator
    {
        ...
        public void TruncateTable()
        {
            ...
            {
                LogService.Instance.Error($"Truncating table failed because DestinationTableName (schemaName) was not specified.");
                return;
            }
            var tableName = (splitResults.Count() > 1) ? splitResults[1] : string.Empty;
            if (string.IsNullOrEmpty(tableName))
            {
                LogService.Instance.Error($"Truncating table failed because DestinationTableName (tableName) was not specified.");
                return;
            }

            LogService.Instance.Info($"Truncating table [{schemaName}].[{tableName}] (if exists)");

            StringBuilder sb = new StringBuilder();
            // Check if the table exists before trying to TRUNCATE it (otherwise it will throw an exception and stop the process)
            sb.AppendLine($"IF (EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '{schemaName}' AND TABLE_NAME = '{tableName}'))");
            sb.AppendLine($"BEGIN");
            sb.AppendLine($"TRUNCATE TABLE [{schemaName}].[{tableName}]");
            sb.AppendLine($"END");
